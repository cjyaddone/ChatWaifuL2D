...
def openaichat(question):
    ...
    userjson_list.append({"role": "user", "content": question})
    #txtrecord.append({"role": "user", "content": question})
    completion = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        temperature=1.2,
        messages=openaijson_list+userjson_list
    )
    #txtrecord.append({"role": "assistant", "content": completion.choices[0].message.content.lstrip("\n")})
    userjson_list.append({"role": "assistant", "content": completion.choices[0].message.content.lstrip("\n")})
