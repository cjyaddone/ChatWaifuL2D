...
class LayerNorm(nn.Module):
  ...
  def forward(self, x):
    x = x.transpose(1, -1)
    x = F.layer_norm(x.to(device), (self.channels,), self.gamma.to(device), self.beta.to(device), self.eps)
...
class ConvReluNorm(nn.Module):
  ...
  def forward(self, x, x_mask):
    ...
    for i in range(self.n_layers):
      x = self.conv_layers[i](x.to(device) * x_mask.to(device))
...
class DDSConv(nn.Module):
    ...
    def forward(self, x, x_mask, g=None):
        ...
        for i in range(self.n_layers):
            y = self.convs_sep[i](x.to(device) * x_mask.to(device))
            y = self.norms_1[i](y)
            y = F.gelu(y)
...
class WN(torch.nn.Module):
  ...
  def forward(self, x, x_mask, g=None, **kwargs):
    ...
    for i in range(self.n_layers):
      x_in = self.in_layers[i](x.to(device))
      if g is not None:
